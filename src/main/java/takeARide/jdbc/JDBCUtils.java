...
public final class JDBCUtils {
	...
	public static <T> int selectForAcconut(Connection c, String sql, String email) throws SQLException {
		try (PreparedStatement ps = c.prepareStatement(sql)) {
			populatePreparedStatement(ps, email);
			ResultSet rs = ps.executeQuery();
			while(rs.next()) {
				if(rs.getString("email").equals(email)) {
					return rs.getInt("is_blocked");
				}
			}
			throw new InternalServerErrorException("Can't validate account: ");
		}
	}

	
	public static <T> int selectForManagerAcconut(Connection c, String sql, String email) throws SQLException {
		try (PreparedStatement ps = c.prepareStatement(sql)) {
			populatePreparedStatement(ps, email);
			ResultSet rs = ps.executeQuery();
			while(rs.next()) {
				if(rs.getString("email").equals(email)) {
					return rs.getInt("is_manager");
				}
			}
			throw new InternalServerErrorException("Can't validate account: ");
		}
